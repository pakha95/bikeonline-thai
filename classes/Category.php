...
class CategoryCore extends ObjectModel
{
    ...
    public function getSubCategories($idLang, $active = true)
    {
        ...
        foreach ($result as &$row) {
            ...
            //display count products by jung
            $categ = new Category((int)$row['id_category'], (int)$idLang);
            $row['nbproducts'] =$categ->getProducts(NULL, 1, 10000,  NULL,  NULL, true);
            //@end
        }
        ...
        */
        if ($getTotal) {
            $sql = 'SELECT COUNT(cp.`id_product`) AS total
					...
					'.Shop::addSqlAssociation('product', 'p').'
					LEFT JOIN `'._DB_PREFIX_.'category_product` cp ON p.`id_product` = cp.`id_product`
					WHERE cp.`id_category` LIKE "'.(int) $this->id.'%"'.
                ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '').
                ($active ? ' AND product_shop.`active` = 1' : '').
                ($idSupplier ? 'AND p.id_supplier = '.(int) $idSupplier : '');
